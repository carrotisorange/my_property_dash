           ...
           <input class="form-control" type="hidden" form="add_billings" name="ctr" value="{{ $ctr++ }}" readonly>     
            ...
            <tr>
              ...
              <td>{{ $item->first_name.' '.$item->last_name }} 
                ...
                <td>
                <?php $prorate_rent =  number_format(Carbon\Carbon::parse($item->moveout_date)->DiffInDays(Carbon\Carbon::now())) ?>
                  @if($item->tenants_note === 'new' )
                  ...
                  @else
                    <input form="add_billings" class="col-md-6" type="number" name="amt{{ $amt_ctr++ }}" step="0.01"  value="{{ prorate_rent }}" oninput="this.value = Math.abs(this.value)">
                  @endif
                </td>
           </tr>
           @endforeach
